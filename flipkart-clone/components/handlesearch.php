...
<body>
    <div class="mx-5 mt-3">
        ...
        <?php
        $query = $_GET['search'] ?? '';
        $sort = $_GET["sort"] ?? 'relevance'; // Default sort is 'relevance'
        ...
        if ($query) {
            ...
            // Append sorting logic based on $sort
            if ($sort === 'priceLowHigh') {
                $sql .= " ORDER BY it.item_price ASC";
            ...
            if ($row > 0) {
                echo '
                <div class="d-flex">
                    <h5 class="fw-semibold mt-1 mx-2">Sorting by</h5>
                    <form class="sort-dropdown" method="GET" action="handlesearch.php">
                        <input type="hidden" name="search" value="' . htmlspecialchars($_GET["search"]) . '">
                        <select name="sort" class="form-select fw-semibold" onchange="this.form.submit()">
                            <option value="relevance" ' . (isset($_GET["sort"]) && $_GET["sort"] == "relevance" ? "selected" : "") . '>Relevance</option>
                            <option value="priceLowHigh" ' . (isset($_GET["sort"]) && $_GET["sort"] == "priceLowHigh" ? "selected" : "") . '>Price -- Low to High</option>
                            <option value="priceHighLow" ' . (isset($_GET["sort"]) && $_GET["sort"] == "priceHighLow" ? "selected" : "") . '>Price -- High to Low</option>
                    ...
                    <div class="row g-0">
                        ...
                        <div class="col-md-3 bg-white d-flex justify-content-between mt-3">
                            <div>
                                ...
                                <span class="d-block">
                                    <small class="fs-6 fw-normal text-body-secondary">
                                        <del>₹' . number_format($noprice) . '</del>&nbsp;
                                    </small>
                                    <span class="text-success fw-semibold">' . $discount . '% off</span>
                                </span>
